...
(define is-empty-board?
  (λ (board)
    (if (member #f (map (lambda (x) (if (or (member RED x) (member YELLOW x));caut elemente de 1 sau 2 in lista
...
(define (getElement list y)
  (if (= y 0)
      (car list)
      (getElement (cdr list) (- y 1))))

(define get-disc
  (λ (board position)
    (let ( (x  (car position)) (y (cdr position)) (aux '()) )
      (getElement (getCol board x) y))))
...
(define (available board aux index) ;in index voi avea de fiecare data indicele coloanei in care ma aflu
  (if (null? board)
      aux
...
(define apply-action
  (λ (state action)
    (let ( (board (get-board state)) (player (get-player state)) )
...
(define (isFull board) ; verifica daca tabla de joc e plina
  (if (member #f (map (lambda (x) (not (member EMPTY x))) board))
      #f
      #t))
  
(define (colWin L index player) ; functie ce verifica daca un player castiga pe o coloana
  (if (= (- (length L) index) 3)
      #f
...
(define (diagPrincipalaHelper board col linie player)
...
(define (diagSecundaraHelper board col linie player)
  (if (and (= player (get-disc board (cons (+ col 1) (- linie 1)))) (= player (get-disc board (cons (+ col 2) (- linie 2))))
             (= player (get-disc board (cons (+ col 3) (- linie 3)))) ( = (get-disc board (cons col linie)) player))
        #t
        #f))
...
(define (checkDiag board x y player)
  (cond
     ((and (= (- (length board) x) 4) (= (- (length (car board)) y) 4)) (if (diagPrincipalaHelper board x y player) #t #f))
    ((and (= (- (length board) x) 4) (> (- (length (car board)) y) 4)) (if (diagPrincipalaHelper board x y player)
                                                                           #t
                                                                           (checkDiag board x (+ y 1) player)))
    ((and (> (- (length board) x ) 4) (= (- (length (car board)) y) 4)) (if (diagPrincipalaHelper board x y player)
                                                                            #t
                                                                           (checkDiag board (+ x 1) 0 player)) )
    ((> (- (length (car board)) y) 4) (if (diagPrincipalaHelper board x y player)
                                          #t
                                          (checkDiag board x (+ y 1) player)))
    (else #f)))
...
(define (checkDiag2 board x y player)
   (cond
      ((and (= (- (length board) x) 4) (= y 3) ) (if (diagSecundaraHelper board x y player) #t #f) )
     ((and (= (- (length board) x) 4) (> y 3) ) (if (diagSecundaraHelper board x y player)
                                                    #t
                                                    (checkDiag2 board x (- y 1) player)) )
    ((and (> (- (length board) x ) 4) (=  y 3)) (if (diagSecundaraHelper board x y player)
                                                    #t
...
(define is-game-over?
  (λ (state)
    (let ( (board (car state)) (player (nextPlayer (get-player state))) )
...
(define (getHelper state aux lista)
  (if (null? lista)
      aux
      (if (equal? (is-game-over? (apply-action state (car lista))) (get-player state))
...
(define get-action
  (λ (state)
    (let* ( (otherPlayer (nextPlayer (get-player state))) (newState (reverse (cons otherPlayer (take state 1)))) )
